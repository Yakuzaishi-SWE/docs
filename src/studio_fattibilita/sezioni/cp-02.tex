...
In particolare, ha aperto le porte a una nuova forma di finanza, la cosiddetta “DeFi” (Finanza Decentralizzata) che ha permesso a chiunque sia dotato di connessione internet di creare un Wallet e possedere quindi criptovalute\glo.
...
\newline
Nel capitolato in questione si vuole proprio risolvere questo problema, in uno scenario che comprende un e-commerce basato su BlockChain in cui si vuole tutelare entrambe le parti coinvolte in un acquisto tramite criptovalute.
...
\begin{itemize}
	\item \textbf{Gestione dell'ordine}:
		\begin{itemize}
			\item Caricamento dell'ordine sulla BlockChain che deve avvenire tramite la signatura di uno {smart contract}\glo tra acquirente e venditore;
			\item Verifica del pagamento da parte dell'acquirente;
			\item Elaborazione e spedizione dell'ordine;
			\item Verifica ricezione dell'ordine tramite QR code usando una clausula dello smart contract;
...
Il comittente non impone nessun vincolo riguardo le tecnologie necessarie ai fini del progetto, ma ha espresso comunque delle preferenze:
\begin{itemize}
	\item \textbf{utilizzo di BlockChain pubblica (come ad esempio Ethereum)} con linguaggio Solidity\glo da usare per la scrittura degli {smart contract}\glo;
...
alternative come Polygon, Avalanche, Fantom e confrontarne eventuali pregi e difetti, con l'obiettivo di un futuro utilizzo reale.
\newline
