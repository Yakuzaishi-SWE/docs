...
\subsection{Verifica} \label{subsection: Verifica}
    ...
    \begin{itemize}
        ...
        \item Devono essere rispettati gli obiettivi di copertura indicati nel Piano di Qualifica.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Walktrough}: Il verificatore esegue un controllo completo del documento alla ricerca di eventuali errori;
        \item \textbf{Inspection}: Il verificatore esegue un controllo mirato ai soli punti del documento nei qualli vi è una più ampia possibilità di errori. Il controllo mirato avviene grazie all'esperienza del verificatore e ad una lista di controllo\glo.
    \end{itemize}
    La tecnica \textbf{Walkthrough} risulta molto onerosa a causa dell’adozione di una ricerca esaustiva degli errori. Questa metologia sarà applicata come fase iniziale di verifica della documentazione. Con l'avanzare dell’attività di progetto ed il ripetersi del processo di verifica sulla documentazione, si stilerà una lista di controllo\glo\ grazie alla quale, successivamente, sarà possibile passare alla tipologia di verifica \textbf{Inspection} che risulta essere più rapida.

    \subsubsection{Verifica del codice}
    Il processo di verifica del codice farà uso di test automatici appositamente creati e si servirà inoltre di analisi statica e dinamica per quanto riguarda il codice scritto ed in particolare:
    \begin{itemize}
        \item \textbf{Analisi statica}: verificatori e programmatoti durante le fasi di stesura e verifica del codice si accertano che siano stati rispettati i proncipi di buona programmazione preimpostati dal team;
